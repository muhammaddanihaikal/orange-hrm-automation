...
class AdminPage:
    def __init__(self, page: Page):
        ...
        self.username = self.field_container.filter(has_text="Username").get_by_role("textbox")
    ...
    def click_edit(self, username):
        self.user_row(username).get_by_role("button").filter(has=self.page.locator("i.bi-pencil-fill")).click()

    # Action langsung Delete tuntas dengan popup
    def delete_user(self, username):
        self.user_row(username).get_by_role("button").filter(has=self.page.locator("i.bi-trash")).click()
