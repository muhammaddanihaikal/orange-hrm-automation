...
class AddUserPage:
    def __init__(self, page: Page):
        ...
        self.employee_name = self.field_container.filter(
        ...
        self.password = self.field_container.filter(
            has=page.get_by_text("Password", exact=True)
        ).get_by_role("textbox")
        self.confirm_password = self.field_container.filter(
    ...
    def add_user(self, data):
        ...
        # isi status
        self.status.click()
        self.page.get_by_role("option", name=data["status"]).click()
